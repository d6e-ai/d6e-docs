...
Plugin はワークスペース側の振る舞いを用意し、カスタム FE はそれを消費します。同じリポジトリで `template.yaml` とフロントエンドを同居させる構成も一般的です。切り分けは [開発パスの選び方](/ja-jp/getting-started/choosing-a-path/) を参照してください。
