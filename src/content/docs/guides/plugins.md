...
| Docker STF | コンテナ（詳細は [Docker STF](/guides/docker-stf/)） |
...
1. [ローカル AI 開発](/guides/local-ai-development/) で実インスタンスに MCP 接続
...
Plugin はワークスペース側の振る舞いを用意し、カスタム FE はそれを消費します。同じリポジトリで `template.yaml` とフロントエンドを同居させる構成も一般的です。切り分けは [開発パスの選び方](/getting-started/choosing-a-path/) を参照してください。
