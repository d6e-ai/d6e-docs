...
フロントエンドは **client secret を持ちません**。本番の callback URL は d6e-auth に登録します（localhost は不要）。詳細は [アーキテクチャ](/getting-started/architecture/) を参照。
...
フロントエンドが依存するテーブル・プロンプト・WF を Plugin としてパッケージ化すると、新しいワークスペースを再現可能に用意できます。切り分けは [開発パスの選び方](/getting-started/choosing-a-path/) を参照してください。
